...
describe("Testing method getPriceWithoutExtras(id)", () => {
	...
	test("Test 2: no car found with given parameter", () => {
		expect(() => register.getPriceWithoutExtras(150)).toThrow(
...
describe("Testing method getTotalPrice(id)", () => {
	...
	test("Test 2: no car found with given parameter", () => {
		expect(() => register.getTotalPrice(150)).toThrow(
			"nothing found with given id"
		);
	});
});
...
describe("Testing method getPriceOfTheExtras(id)", () => {
	...
	test("Test 1: Get the total price of extras with parameter 2", () => {
		expect(register.getPriceOfTheExtras(2)).toEqual(210);
	});

	test("Test 2: No extras found with given parameter", () => {
		expect(register.getPriceOfTheExtras(3)).toBe(0);
	});

	test("Test 3: No car with given id found", () => {
		expect(() => register.getPriceOfTheExtras(150)).toThrow(
			"nothing found with given id"
		);
	});
	...
	describe("Testing method hasAccessories(id)", () => {
		...
		test("Returns true if car has accessories", () => {
			expect(register.hasAccessories(2)).toBe(true);
		});

		test("Test 2: Returns false if car doesn't have accessories", () => {
			expect(register.hasAccessories(3)).toBe(false);
		});

		test("Test 3: Returns false if parameter is missing", () => {
			expect(register.hasAccessories()).toBe(false);
		});
	});
});
